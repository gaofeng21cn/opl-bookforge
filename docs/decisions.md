...
- Add a Book Forge project hygiene helper for concrete workflow regressions found during the 200-page manuscript run.
...
- Keep App/default product exposure separate until a product owner decision and App-owned contracts add Book Forge to visible default routes.
...
- When the owner supplies a stronger PDF/proof artifact as a reference version, Book Forge must compare front matter, proof label, page geometry, image placement, chapter punctuation, and table-of-contents hierarchy as production quality inputs, not only body prose.
...
- Reject importing external runtimes such as generic multi-agent orchestrators, Dify/CrewAI-style controllers, private vector stores, hidden schedulers, or second truth sources. OPL keeps runtime/projection ownership; OPL Book Forge keeps manuscript, evidence, memory, quality, export, and owner-gate truth.

## 2026-06-20: Complete-Version Reference Absorption

- When the owner compares two full-book review PDFs or says another version is more publishable, Book Forge must run a complete-version absorption pass, not a local wording pass.
...
- If the owner says the active manuscript has improved but still feels like a compressed review edition, Book Forge records a `publication_book_feel_gap`. Formal publication manuscripts need reader walking space: concrete entry scenes, stakeholder tension, explanatory transitions, short classroom/workplace/project examples, figure/table interpretation, and chapter handoffs. Cleaner headings or sharper model names alone do not absorb this critique.
- High judgment density is a craft defect for formal nonfiction when readers receive too many finished conclusions without enough process. The repair is to add evidence-bearing micro-cases, process beats, and connective explanation at concept boundaries, while still cutting repeated central judgments that add no new evidence, mechanism, model use, or handoff.
- Figure style is a publication-design decision. Book Forge must choose and record a whole-book visual stance, such as `light_methodology_sketchnote` or `formal_framework_diagram`, before final figure regeneration or proof claims, and then keep prompts, captions, and rendered-page inspection consistent.
- If repeated complete-version comparisons say the active `completed` manuscript is best as a concise/electronic/internal review edition but a fuller reference remains stronger for formal publication, Book Forge must stop the local-polish loop and record `target_artifact_choice`. Concise review routes keep their sharper argument and explicit review/e-book labeling; formal publication routes must promote the fuller base or create a formal expansion queue before claiming the publication-level critique absorbed.

## 2026-06-20: OPL-Owned Publication Proof Dependency Route

- Book Forge publication proof depends on local Pandoc, XeLaTeX, Poppler, and TeX Live package availability, but dependency diagnosis and maintenance belong to the OPL system layer.
...
- Book Forge may use the doctor result as proof/export gate input, but it must not implement a private OS package manager, TeX package installer, runtime scheduler, or second dependency truth source inside this repo.
...
- The bundled proof header removed `titling.sty` and `tocloft.sty`; those packages are legacy diagnostics only and must not be required by the current Book Forge proof profile.

## 2026-06-20: OPL Artifact Lifecycle For Book Workspaces

- Book Forge keeps manuscript, source-map bodies, memory bodies, figures, exports, quality reports, typed blockers, and owner receipts as domain-owned workspace refs.
...
- Add a completion audit split inside the Temporal consumption policy. OPL provider completion, generated-surface readiness, and StageRun status readiness are transport/runtime accounts only; they do not count as `review_pdf`, `publication_proof`, `final_export`, owner acceptance, or Book Forge domain completion.
- Keep `review_pdf`, `publication_proof`, `final_export`, and `owner_acceptance` as separate Book Forge completion accounts. Review PDFs need review artifact refs, publication proofs additionally need publication design, asset-resolution, and rendered-page inspection refs, and final export additionally needs an `owner_export_acceptance_ref`. Owner acceptance is not inferred from StageRun status or generated interface state.
- Treat the real short-book pilot as historical evidence and acceptance tail, not completion closure. It can show workflow and artifact plumbing, but live StageRun/runtime claims still require live StageRun evidence, and final export claims still require owner/export acceptance.
...
- Keep Book Forge's visual and runtime identity separate. Book Forge must not copy Kami's parchment/ink-blue visual language, HTML templates, WeasyPrint route, font downloader, plugin update workflow, or any private proof truth source.
- Add publication design tokens as Book Forge-owned proof refs covering page geometry, typography, line height, measure, captions, tables, callouts, figures, headers/footers, page numbering, color restraint, density thresholds, and rendered-page expectations.
