...
- Figure style is a publication-design decision. BookForge must choose and record a whole-book visual stance, such as `light_methodology_sketchnote` or `formal_framework_diagram`, before final figure regeneration or proof claims, and then keep prompts, captions, and rendered-page inspection consistent.
- If repeated complete-version comparisons say the active `completed` manuscript is best as a concise/electronic/internal review edition but a fuller reference remains stronger for formal publication, BookForge must stop the local-polish loop and record `target_artifact_choice`. Concise review routes keep their sharper argument and explicit review/e-book labeling; formal publication routes must promote the fuller base or create a formal expansion queue before claiming the publication-level critique absorbed.
