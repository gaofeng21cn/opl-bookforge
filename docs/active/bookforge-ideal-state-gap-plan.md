...
2026-06-30 SSOT refresh: this plan keeps Book Forge functional / structural gaps only: standard scaffold/interface shape, golden-path routing, stage pack refs, generated/hosted surface consumption, PDF/proof helper plumbing, revision-entrypoint routing, workspace artifact-lifecycle handoff refs, default-caller structural gates, no-active-caller/tombstone/provenance cleanup, and no-second-truth guards. Real long-book runs, owner acceptance, publication-proof visual acceptance, final export acceptance, live OPL StageRun / hosted artifact-handoff parity, physical delete authorization, and production-ready claims are later evidence lanes. They must not be mixed into this active gap list, and scaffold/interface validation, pilot exports, rendered pages, docs foldback, OMA evidence, or helper plumbing cannot replace them.
...
- OPL-generated or OPL-hosted surfaces carrying descriptors, lifecycle refs, artifact-lifecycle projections, StageRun attempt requests, and interface metadata only.
...
| PDF/proof helper plumbing | `helper_plumbing_landed` | `runtime/native_helpers/bookforge_pdf_export.py`, helper doctor/profile checks in `scripts/verify.sh`, compile/render smoke in `scripts/verify.sh pdf-smoke` | Default verification keeps helper plumbing and policy checks fast; compile/render proof-backend behavior is explicit. Helper evidence supports proof plumbing, not final visual judgment. |
...
| Live StageRun progress evidence contract | `owner_typed_blocker_recorded_not_ready_claim` | `contracts/live_stage_run_progress_evidence.json`, historical pilot typed-blocker refs, focused policy test, OPL conformance readback | Provides the domain-owned refs-only evidence contract expected by OPL conformance and binds current progress to typed blockers; it is not live progress completion, domain readiness, production readiness, final export acceptance, owner acceptance, or runtime ownership. |
...
| Private platform retirement matrix | `non_live_tail_matrix_landed_not_owner_or_live_claim` | `contracts/functional_privatization_audit.json#bookforge_private_platform_retirement_matrix`, `contracts/generated_surface_handoff.json#private_platform_retirement_projection`, focused policy test | Publication/export helper, image asset helper, project hygiene helper, and runtime/session/update absence are classified as domain-specific helper refs or no-active private platform surfaces. This proves only non-live structural routing; owner/live acceptance, hosted parity, publication/final-export readiness, and physical delete authorization remain open. |
...
| Live StageRun progress evidence tail | Book Forge now exposes a refs-only live progress evidence contract for OPL conformance, with typed blockers for owner acceptance / final export / live StageRun evidence tail still open. | Keep `contracts/live_stage_run_progress_evidence.json` bound to domain-owned refs and false-ready flags; route actual live progress closure to future owner route-back, no-regression, long-soak, or owner acceptance evidence. |
...
| Private platform tail retirement | Publication/export helper, image asset helper, project hygiene helper, and runtime/session/update/workbench absence now have an explicit non-live tail matrix and generated/hosted projection refs. | Maintain `contracts/functional_privatization_audit.json#bookforge_private_platform_retirement_matrix`, `contracts/generated_surface_handoff.json#private_platform_retirement_projection`, and the focused policy test; route owner acceptance, live hosted parity, final export, publication readiness, and physical delete authorization to later evidence lanes. |
| StageRun default entry closure | Revision routing, publication/final-export, and production acceptance now share `contracts/temporal_stage_run_consumption_policy.json#default_entry_routing`; `contracts/action_catalog.json` uses `opl_stage_run_attempt_request` for default CLI/product-entry projection instead of direct repo-local `opl-bookforge` runner commands. | Maintain StageRun account/default owner-boundary fields in `temporal_stage_run_consumption_policy`, `action_catalog`, `generated_surface_handoff`, `stage_run_kernel_profile`, and production-acceptance contracts; route live execution/parity to later evidence lanes. |

本轮按 `launcher|wrapper|alias|compat|legacy|status|sidecar|product-entry|helper|facade` 关键词复核后，没有发现可直接物理删除的孤立普通入口。命中的 helper/projection/default-entry surfaces 已在 functional privatization audit、generated surface handoff、Temporal StageRun consumption policy 和本计划中归为 domain-specific helper refs、refs-only projection 或 StageRun account dispatch metadata；退役仍必须满足 OPL generated/hosted parity、no-active-caller、no-forbidden-write、tombstone/provenance、owner acceptance 或 physical delete authorization。
