...
| PDF/proof helper plumbing | `helper_plumbing_landed` | `runtime/native_helpers/bookforge_pdf_export.py`, OPL native-helper probe/profile checks in `scripts/verify.sh`, compile/render smoke in `scripts/verify.sh pdf-smoke` | Default verification keeps helper plumbing and policy checks fast; compile/render proof-backend behavior is explicit. Helper evidence supports proof plumbing, not final visual judgment. |
| Image generation executor boundary | `opl_executor_cutover_landed` | `contracts/image_generation_executor_handoff.json`, `runtime/native_helpers/bookforge_imagegen_asset.py`, focused fake-OPL adapter test | OPL Runway owns Codex discovery, required-capability activation, process transport, and executor receipt. Book Forge retains the prompt, project-local target, bitmap validation, figure receipt, manifest sync, and publication/delivery authority. Structural proof does not count as a live provider generation or visual acceptance. |
...
| Live StageRun progress evidence contract | `topology_superseded_live_evidence_deferred_not_ready_claim` | `contracts/live_stage_run_progress_evidence.json`, immutable historical pilot/OMA provenance, focused topology and policy tests | Records that old pilot evidence predates the active five-stage topology and cannot be treated as current-stage evidence. Current-topology Live Evidence remains deferred; no readiness, final-export, owner-acceptance, or runtime claim is made. |
