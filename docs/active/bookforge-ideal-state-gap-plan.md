...
| OPL artifact lifecycle integration | OPL refs-only transport exists as local source/test evidence; Book Forge has repo-source byproduct guard coverage for ignored/generated residue, but no real workspace apply receipt for review-repair transport and refreshed downstream refs. | Run `opl workspace artifact-lifecycle --apply` against a real Book Forge workspace with `handoff/review-repair-transport.json`. |
...
| Lifecycle/currentness readback | Book-length workspaces need refs-only lifecycle/currentness projection before handoff/readiness claims. Repo-source byproduct hygiene now prevents ignored residue from masquerading as clean source state, but it is not workspace lifecycle evidence. | Fresh OPL artifact-lifecycle apply/readback over Book Forge project refs. |
