...
- The 2026-06-19 publication-style slice adds a bundled `bookforge-zh-publication-proof` Pandoc/XeLaTeX profile for Chinese nonfiction e-book proof output. The profile is verified through `scripts/verify.sh` with rendered nonblank proof pages, but this still does not replace owner acceptance or book-specific publication review.
