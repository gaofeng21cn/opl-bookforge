...
For final manuscript figures requiring new artwork, use OPL-hosted Codex `imagegen` / native image generation. OPL owns capability activation, executor transport, workspace output allocation, bitmap materialization, attempt/output refs, and persistence. The Book Forge image handler only consumes host-injected refs, a workspace-relative bitmap path plus SHA-256, and figure metadata; it validates the contained regular bitmap and returns a figure-authority receipt candidate or quality debt. It must not create execution requests, spawn OPL/Codex, generate/copy bitmap bytes, or persist receipts/manifests. Chat-preview-only images, missing paths, placeholders, or invalid assets are image-asset quality debt: they close figure-ready/publication-proof/final-export claims but do not block later stage work. Direct provider/API-key calls are outside the Book Forge handler boundary.
