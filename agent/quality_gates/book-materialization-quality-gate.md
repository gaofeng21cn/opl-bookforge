...
- When repeated complete-version feedback says the active manuscript is the best concise/electronic/internal review edition but a fuller reference remains stronger for formal publication, the materialization package records `target_artifact_choice` before further manuscript repair. A concise route must stay labeled as review/trial/e-book output and cannot support formal-publication claims; a formal route must promote or rebuild from the fuller publication base or an expansion queue rather than continuing local polish of a compressed review manuscript.
...
- Repeated comparisons keep returning the same route verdict, for example "best concise/electronic edition, formal publication should use the fuller reference", while BookForge continues fast-track wording passes without recording a target artifact route or formal-publication base decision.
