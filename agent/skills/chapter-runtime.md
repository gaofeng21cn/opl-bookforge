...
- When the book project is indexed by an OPL workspace, refresh `opl workspace artifact-lifecycle --apply` after chapter source refs, book-memory refs, figure manifests, review PDFs, metrics, hygiene reports, or other current output refs change. Chapter readiness claims may cite OPL refs-only lifecycle health, but OPL health does not replace BookForge chapter QC or owner acceptance.
